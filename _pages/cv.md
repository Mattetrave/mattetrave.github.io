---
layout: archive
title: "CV"
permalink: /cv/
author_profile: true
redirect_from:
  - /resume
---

{% include base_path %}

## Profile
M.Sc. graduate in Biomedical Engineering specialized in Biomedical Instrumentation, experienced in the development of visuo-cognitive assessment frameworks using research-grade eye-tracking systems. Driven by deep curiosity and a commitment to continuous learning, I am highly motivated to join an R&D team or collaborate with clinical institutions (including radiotherapy systems) to drive innovation in active medical devices.

---

## Education
**Master's Degree in Biomedical Engineering** — Politecnico di Torino, Turin (2023 – 2026)

Specialization: Biomedical Instrumentation

Thesis: *"Beyond paper and pencil: validation of a multimodal technological framework for evaluation of pediatric visuospatial attention"*

Final grade: 106/110

**Bachelor's Degree in Biomedical Engineering** — Politecnico di Torino, Turin (2019 – 2023)

Final grade: 94/110

**High School Diploma** — Liceo Scientifico Edoardo Amaldi, Novi Ligure (AL) (2019)

Specialization: Scientifico tradizionale — Final grade: 100/100

---

## Experience
**Guest Researcher – Evaluation of pediatric visuospatial attention — IIT Unit for Visually Impaired People (U-VIP), Genoa (Nov 2025 – Jul 2026)**

Master's thesis research on validation of a multimodal framework to assess pediatric visuospatial attention, overcoming limitations of the traditional paper-based Modified Bells Test.

* Programmed the IIT TechPAD multisensory prototype and designed a digitized assessment environment similar to Modified Bells Test synchronized with a Tobii Pro Spectrum eye-tracker.
* Managed real-time data acquisition protocols and led on field experimental sessions with a cohort of 18 typically developing children (aged 4–12).
* Engineered custom pipelines in MATLAB and Python for gaze data processing, advanced statistical evaluation, and developed both supervised and unsupervised machine learning analysis to characterize visual and cognitive symptoms of fatigue.
* Collaborated within a highly specialized R&D team (supervised by Dr. M. Gori and Dr. E. Montagnani), actively defining project specifications and optimizing methodologies during technical meetings.

**Curricular Intern – Ultrasound and radiofrequency surgical devices — Ospedale Cottolengo, Turin (Sept 2022 – Dec 2022)**

Bachelor's degree internship analyzing and comparing two surgical devices: Harmonic 1100 (Ethicon, ultrasound) and LigaSure (Medtronic, radiofrequency).

* Attended live surgical procedures in the operating room to directly observe the instruments in action and evaluate their practical applications.
* Conducted structured interviews with head surgeons to collect qualitative feedback on device performance, handling, and adaptability across diverse intraoperative scenarios.
* Collaborated within a two-person team to execute the project, successfully managing strict deadlines while interfacing effectively with multiple hospital departments and medical staff.

---

## Skills

**Programming:** MATLAB, Python

**Proficient with:** Tobii SDK, TechPAD software, LaTeX, Microsoft Office, GitHub

**Familiar with:** Unity 6, GIMP, SolidWorks, AVR Assembly, C, Meshlab

---

## Languages

* Italian: Native
* English: Europe Level B2 (Cambridge English Level 1 Certificate in ESOL International - First)

---

## Publications
  <ul>{% for post in site.publications reversed %}
    {% include archive-single-cv.html %}
  {% endfor %}</ul>

---

## Additional Information
**Volunteering**

* Humanitarian Mission — Casa di Carità Umbra, Leskoc, Kosovo (Aug 2026)
  * Week-long mission organized by the Diocese of Tortona, supporting a residential care facility in a complex socio-economic context.
  * Carried out facility maintenance work and agricultural tasks alongside local youth, and provided full-time practical and relational support to children hosted at the facility.
* Active blood donor at FIDAS of Novi Ligure (AL) since 2023.

**Interests**

Alpine skiing and weight training, with particular attention to biomechanics and correct exercise execution · Piano (playing since age 8; weekly lessons until 2020) · Science dissemination — regular reader of *National Geographic* and *Nature*

---

## Download